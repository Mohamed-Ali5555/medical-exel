...
class ProductController extends Controller
{
    ...
    public function duplicate(Request $request){  
        ...
        if($current_product_count > 0){
            echo "<script>
                toastr.warning('تم إضافه هذا الصنف إلي متجرك مسبقاً');
                </script>";
        }else{ 
            $product = Product::find($request->id); // Get the original model by ID

            $newModel = $product->replicate(); // Make a copy of the original model
            $newModel->parent_id = $request->id; // Set the new parent_id value
            $newModel->user_id = auth('seller')->id(); // Set the new parent_id value
            $newModel->added_by = "seller"; // Set the new added_by value using the currently authenticated user's ID
            unset($newModel->reviews_count);
            $newModel->save(); // Save the new model to the database
            echo "<script>
                toastr.success('تم إضافه الصنف إلي متجرك');
                </script>";
        }
