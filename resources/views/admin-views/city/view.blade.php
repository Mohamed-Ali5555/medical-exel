    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-1 text-capitalize d-flex align-items-center gap-2">
                <img width="20" src="{{ asset('/public/assets/back-end/img/city.png') }}" alt="">
        ...
        <!-- Content Row -->
        <div class="row pb-4 d--none" id="main-city"
            style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0 text-capitalize">{{ \App\CPU\translate('city_form') }}</h5>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <label for="name"
                                            class="title-color text-capitalize">{{ \App\CPU\translate('city_name') }}</label>
                                ...
                                <div class="col-md-6">
                                    <label for="name" class="title-color">{{ \App\CPU\translate('goverment') }}</label>
                                    <select class="js-example-basic-multiple form-control" name="goverment_id"
                                        onchange="getRequest('{{ url('/') }}/admin/product/get-categories?parent_id='+this.value,'sub-category-select','select')"
                                        required>
                                        <option value="{{ old('goverment_id') }}" selected disabled>
                                            ---{{ \App\CPU\translate('Select') }}---</option>
                                        @foreach ($goverments as $goverment)
                                            <option value="{{ $goverment['id'] }}"
                                                {{ old('name') == $goverment['id'] ? 'selected' : '' }}>
                                                {{ $goverment['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-8 col-lg-6">
                                <div
                                    ...
                                    <form action="{{ url()->current() }}" method="GET">
                                        <div class="input-group input-group-merge input-group-custom">
                                            ...
                                            <input id="datatableSearch_" type="search" name="search" class="form-control"
                                                placeholder="{{ \App\CPU\translate('Search_by_city_Type') }}"
                                    ...
                                    <div id="city-btn">
                                        <button id="main-city-add" class="btn btn--primary text-nowrap">
                                            <i class="tio-add"></i>
                                            {{ \App\CPU\translate('city') }}
                                        </button>
                                    </div>
                    ...
                    <div class="table-responsive">
                        <table id="columnSearchDatatable"
                            ...
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    ...
                                    <th>{{ \App\CPU\translate('name') }}</th>
                                              <th>{{ \App\CPU\translate('goverment name') }}</th>
                            ...
                            @foreach ($cities as $key => $city)
                                <tbody>
                                    <tr id="data-{{ $city->id }}">
                                        <td class="pl-xl-5">{{ $cities->firstItem() + $key }}</td>
                                        <td>{{ \App\CPU\translate(str_replace('_', ' ', $city->name)) }}</td>


                                        <td>{{ \App\CPU\translate(str_replace('_', ' ', $city['goverment']['name']??'not goverment')) }}</td>

<td>{{$city['goverment']['name']??''}}</td>
                                        {{-- <td>{{ $city->branch->gover_name }}</td> --}}
                                        ...
                                        <td class="text-center">
                                            <label class="mx-auto switcher">
                                                <input type="checkbox" class="status switcher_input"
                                                    id="{{ $city['id'] }}" {{ $city->status == 1 ? 'checked' : '' }}>
                                        ...
                                        <td>
                                            <div class="d-flex gap-10 justify-content-center">
                                                ...
                                                <a class="btn btn-outline-danger btn-sm cursor-pointer delete"
                                                    title="{{ \App\CPU\translate('Delete') }}" id="{{ $city['id'] }}">
...
@push('script')
    ...
    <script>
        $('#main-city-add').on('click', function() {
            $('#main-city').show();
        });

        $('.cancel').on('click', function() {
            $('.banner_form').attr('action', "{{ route('admin.city.store') }}");
            $('#main-city').hide();
        });
